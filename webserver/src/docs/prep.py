...
# parser.add_argument("--srcstorageconstring", help="Azure Blob Storage account name")
# parser.add_argument("--sourcecontainer", help="Azure Blob Storage container name")
...
# class DocContainer():
#     """Document interface class for container blob storage in Azure"""
#     def __init__(
#             self, 
#             client, 
#             container
...
# document_analysis_client = DocumentAnalysisClient(document_analysis_endpoint, document_analysis_key, headers={"x-ms-useragent": "azure-search-chat-demo/1.0.0"})

# verbose = os.getenv("VERBOSE_FLAG")
# removeall = os.getenv("REMOVEALL_FLAG")
# remove = os.getenv("REMOVE_FLAG")
# skipblobs = os.getenv("SKIPBLOBS_FLAG")
# localpdfparser = os.getenv("LOCALPDFPARSER_FLAG")
# category=None
...
def upload_blobs(filename, client):
...
def get_document_text(filename, client):
    offset = 0
    page_map = []
    if localpdfparser:
        reader = PdfReader(filename)
...
def split_text(page_map, filename):
    ...
    while start + SECTION_OVERLAP < length:
        ...
        #print("text for a section ",section_text)
...
def create_search_index(search_index_client):
    if os.getenv("VERBOSE_FLAG"): print(f"Ensuring search index {os.getenv('AZURE_SEARCH_INDEX')} exists")
    # index_client = SearchIndexClient(endpoint=f"https://{args.searchservice}.search.windows.net/",
    #                                  credential=search_creds)
    if os.getenv("AZURE_SEARCH_INDEX") not in search_index_client.list_index_names():
        index = SearchIndex(
...
def remove_from_index(filename, search_client):
    # if verbose: print(f"Removing sections from '{filename or '<all>'}' from search index '{args.index}'")
    # search_client = SearchClient(endpoint=f"https://{args.searchservice}.search.windows.net/",
    #                                 index_name=args.index,
    #                                 credential=search_creds)
    while True:
...
#     blob_service_client = BlobServiceClient.from_connection_string(args.srcstorageconstring)
#     return blob_service_client

def get_blob_list(client, blob_format=".pdf"):
    """
        Returns only blobs with .pdf
    """
    blob_service = client
    blob_container = blob_service.get_container_client(os.getenv("AZURE_SOURCE_CONTAINER"))
    blobs = list(filter(lambda bname:bname.lower().endswith(".pdf"),blob_container.list_blob_names()))
    return blobs

def download_blob_to_local(filename, client):
    print(os.getcwd())
...
def docs_run():
    ...
    if removeall:
        ...
        for filename in get_blob_list(client=source_blob_client):
            filename = f"./src/docs/data/{filename}"
            print(f"Downloading file '{filename}' to local ")
            download_blob_to_local(filename, client=source_blob_client)
            ...
            if remove:
                ...
                if not skipblobs:
                    upload_blobs(filename, client=sink_blob_client)
                page_map = get_document_text(filename, client=document_analysis_client)
                sections = create_sections(os.path.basename(filename), page_map)
                index_sections(os.path.basename(filename), sections, search_client=search_client)
            print(f"Removing file '{filename}' from local ")
            try:
                os.remove(filename)
                print(f"SUCCESS : Removed file '{filename}' from local...")
            except Exception as e:
                print(f"FAILURE : Removed file '{filename}' from local...\n cause {e}")
        
    return "DATA_SYNC_COMPLETED"